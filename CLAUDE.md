# CLAUDE.md — Cashflow Workbook Generator
...
Detailed logic and rules live in `.claude/`. Read the relevant file before touching that area of the code.

| File | When to read it |
|---|---|
| `.claude/architecture.md` | Tool structure, CDN constraints, library usage |
| `.claude/input-files.md` | Parsing BOQ, Budget, Assumptions input files |
| `.claude/cash-in-logic.md` | Cash In hierarchy, BETA.DIST distribution, column layout |
| `.claude/cash-out-logic.md` | Cash Out groups, milestone sub-rows, special sections |
| `.claude/output-structure.md` | Sheet list, Cashflow sheets, Summary, EQU |
| `.claude/styling.md` | Colors, fonts, borders, number formats |
| `.claude/pitfalls.md` | Known bugs, hard-won lessons, deferred features |
